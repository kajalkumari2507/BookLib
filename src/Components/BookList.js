  ...
  function Book(book, index) {
    ...
    return (
      ...
      >
        ...
        {book.book.volumeInfo.imageLinks && book.book.volumeInfo.imageLinks.thumbnail ? (
          <img
          ...
          <img
            src="http://books.google.com/books/content?id=L18VBQAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api" // Replace this with your default image URL
            alt=""
            className="book-image"
          />
        )}
  ...
  function BookImage(book, index) {
    ...
    return (
      <div
        ...
        key={index}
      >
        <img src={imgg} alt="imagee" className="only-book-image" />
      </div>
    );
  }
  return (
    <>
      ...
      ) : books.length > 0 ? ( // Check if books array has items
        <div className="book-list">
          {books.map((book, index) =>
            index < 3 ? <Book book={book} index={index} /> : ""
          )}
        </div>
      ) : ( // Display "Not Found" message if no books found
